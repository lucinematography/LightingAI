...
function resolveFixture(e = {}) {
  ...
  const exact = FIXTURE_LIBRARY.find(f => {
    ...
    const modelWithoutLs = model.replace(/^ls/, "");
    const fullName = normalizeEquipmentName(`${f.manufacturer || ""} ${f.model || ""}`);
    return equipmentName === model || equipmentName === modelWithoutLs || equipmentName === fullName;
  });
...
app.listen(port, () => console.log(`LIGHTING AI backend running on port ${port}`));
