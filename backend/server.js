...
function accessoryTreeForFixture(fixtureId) {
  const direct=ACCESSORY_LIBRARY.filter(a=>(a.compatibleWith||[]).includes(fixtureId));
  const directIds=new Set(direct.map(a=>a.id));
  const dependent=ACCESSORY_LIBRARY.filter(a=>(a.compatibleWith||[]).some(id=>directIds.has(id)));
  return {
    direct: direct.map(a=>`${a.manufacturer} ${a.model} [${accessoryDetails(a,fixtureId)}]`),
    dependent: dependent.map(a=>`${a.manufacturer} ${a.model} [${accessoryDetails(a)}] requires: ${(a.compatibleWith||[]).filter(id=>directIds.has(id)).join(' or ')}`)
  };
}
function formatEquipmentForAI(equipment = []) {
  return equipment.map(e => {
    ...
    return `${e.name} x${e.qty||1}${specs?` [${specs}]`:""}${accessories.direct.length?` | Compatible accessories: ${accessories.direct.join("; ")}`:""}${accessories.dependent.length?` | Dependent accessories (require a compatible parent modifier): ${accessories.dependent.join("; ")}`:""}`;
  }).join("; ");
}
