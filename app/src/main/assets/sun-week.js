(function(){
  'use strict';
  const TXT={
    sr:{title:'Pregled narednih 7 dana',note:'Poređenje izlaska, zalaska, golden hour-a i maksimalne visine Sunca. Dodirni datum da ga primeniš.',rise:'IZLAZAK',set:'ZALAZAK',goldAm:'GOLDEN JUTRO',goldPm:'GOLDEN VEČE',max:'MAKS. VISINA',apply:'PRIMENI'},
    en:{title:'Next 7 days overview',note:'Compare sunrise, sunset, golden hour and maximum Sun elevation. Tap a date to apply it.',rise:'SUNRISE',set:'SUNSET',goldAm:'GOLDEN AM',goldPm:'GOLDEN PM',max:'MAX ELEVATION',apply:'APPLY'}
  };
  ...
  function render(){
    ...
    for(let i=0;i<7;i++){
      ...
      rows.push('<button type="button" class="sun-week-row" data-date="'+dateValue(d)+'"><span class="sun-week-date"><b>'+dateLabel(d)+'</b><small>'+dateValue(d)+'</small></span><span><small>'+tx.rise+'</small><b>'+fmt(c.sunrise)+'</b></span><span><small>'+tx.set+'</small><b>'+fmt(c.sunset)+'</b></span><span><small>'+tx.goldAm+'</small><b>'+range(w.goldenMorningStart,w.goldenMorningEnd)+'</b></span><span><small>'+tx.goldPm+'</small><b>'+range(w.goldenEveningStart,w.goldenEveningEnd)+'</b></span><span><small>'+tx.max+'</small><b>'+s.maxElevation.toFixed(1)+'°</b></span><em>'+tx.apply+' ›</em></button>');
    }
