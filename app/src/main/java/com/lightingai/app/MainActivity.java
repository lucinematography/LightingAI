...
public class MainActivity extends Activity {
    ...
    private void installCatalogView() {
        ...
        String js = "(function(){" +
            ...
            "function accessoryMatchesFixture(a,fid){var c=a.compatibleWith||[];if(c.indexOf(fid)>=0)return true;var s=JSON.stringify(a);return s.indexOf(fid)>=0;}" +
            "function fixtureRows(list,sr,accent){var h='';list.forEach(function(f){var on=isSelected(f.id);h+='<div style=\\\"margin-bottom:8px\\\"><button class=\\\"btn '+(on?'primary':'secondary')+'\\\" style=\\\"width:100%;text-align:left;margin:5px 0\\\" onclick=\\\"toggleCatalogItem(\\\'fixture\\\',\\\''+esc(f.id)+'\\\')\\\">'+(on?'✓ ':'＋ ')+esc(nameFixture(f))+'</button>';if(on){var own=window.catalogAccessories.filter(function(a){return maker(a)===maker(f)&&accessoryMatchesFixture(a,f.id);});h+='<div style=\\\"margin:4px 0 12px 12px;padding:10px;border-left:2px solid '+accent+'\\\"><div class=\\\"muted small\\\" style=\\\"margin-bottom:6px\\\">'+(sr?'DODACI ZA ':'ACCESSORIES FOR ')+esc(nameFixture(f))+' ('+own.length+')</div>';if(own.length){own.forEach(function(a){var aon=isSelected(a.id);h+='<button class=\\\"btn '+(aon?'primary':'secondary')+'\\\" style=\\\"width:100%;text-align:left;margin:4px 0\\\" onclick=\\\"toggleCatalogItem(\\\'accessory\\\',\\\''+esc(a.id)+'\\\')\\\">'+(aon?'✓ ':'＋ ')+esc(nameAccessory(a))+'</button>';});}else h+='<div class=\\\"muted small\\\">'+(sr?'Nema dodataka u katalogu za ovaj reflektor.':'No catalog accessories for this fixture.')+'</div>';h+='</div>';}h+='</div>';});return h;}" +
            "function accessoryRows(list,sr){var h='';list.forEach(function(a){var on=isSelected(a.id);var compat=(a.compatibleWith||[]).join(', ');h+='<div style=\\\"margin-bottom:8px\\\"><button class=\\\"btn '+(on?'primary':'secondary')+'\\\" style=\\\"width:100%;text-align:left\\\" onclick=\\\"toggleCatalogItem(\\\'accessory\\\',\\\''+esc(a.id)+'\\\')\\\">'+(on?'✓ ':'＋ ')+esc(nameAccessory(a))+'</button>'+(compat?'<div class=\\\"muted small\\\" style=\\\"padding:4px 8px\\\">'+(sr?'Kompatibilno sa: ':'Compatible with: ')+esc(compat)+'</div>':'')+'</div>';});return h;}" +
            ...
            "window.renderFullCatalog=function(){var box=document.getElementById('equipmentList');if(!box)return;var old=document.getElementById('catalogSearch');var q=old?old.value:'';q=q.toLowerCase();var sr=(window.currentLang||'sr')==='sr';var allF=window.catalogFixtures.filter(function(x){return JSON.stringify(x).toLowerCase().indexOf(q)>=0;});var allA=window.catalogAccessories.filter(function(x){return JSON.stringify(x).toLowerCase().indexOf(q)>=0;});var af=allF.filter(function(x){return maker(x).indexOf('aputure')>=0;});var aa=allA.filter(function(x){return maker(x).indexOf('aputure')>=0;});var rf=allF.filter(function(x){return maker(x).indexOf('arri')>=0;});var ra=allA.filter(function(x){return maker(x).indexOf('arri')>=0;});" +
