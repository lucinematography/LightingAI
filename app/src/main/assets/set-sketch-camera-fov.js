...
const TXT={
  sr:{title:'KADAR KAMERE / FOV',focal:'ŽIŽNA DALJINA (mm)',sensor:'ŠIRINA SENZORA (mm)',angle:'HORIZONTALNI FOV',width3:'ŠIRINA KADRA NA 3 m',width5:'ŠIRINA KADRA NA 5 m',hint:'Geometrijski horizontalni kadar na tlocrtu. Stvarna slika zavisi i od crop-a, aspect ratio-a i framinga.'},
  en:{title:'CAMERA FRAME / FOV',focal:'FOCAL LENGTH (mm)',sensor:'SENSOR WIDTH (mm)',angle:'HORIZONTAL FOV',width3:'FRAME WIDTH AT 3 m',width5:'FRAME WIDTH AT 5 m',hint:'Geometric horizontal frame on the floor plan. The real image also depends on crop, aspect ratio and framing.'}
};
const t=()=>TXT[lang()];
...
function cameraCone(scene,o){
  const focal=clamp(Number(o.focalLengthMm)||35,8,300);
  const sensor=clamp(Number(o.sensorWidthMm)||36,5,70);
  const angle=fov(sensor,focal);
  const r=(o.rot||0)*Math.PI/180,dx=Math.sin(r),dy=-Math.cos(r),distance=rayToEdge(scene,o,dx,dy);
  if(!(distance>0))return '';
  const half=distance*Math.tan((angle*Math.PI/180)/2),px=-dy,py=dx;
  const p0=screenXY(scene,o.x,o.y);
  const p1=screenXY(scene,o.x+dx*distance+px*half,o.y+dy*distance+py*half);
  const p2=screenXY(scene,o.x+dx*distance-px*half,o.y+dy*distance-py*half);
...
function draw(){
  const svg=E('setSketchSvg'),bundle=activeBundle();
  ...
  bundle.scene.objects.filter(o=>o.type==='camera').forEach(o=>{
    ...
    const wrap=document.createElementNS('http://www.w3.org/2000/svg','g');wrap.innerHTML=markup;
...
function enhanceSelected(){
  ...
  const picked=selectedCamera();
  const existing=E('setSketchCameraFovBox');
  if(!picked){if(existing)existing.remove();return;}
  const camera=picked.camera;
  if(existing)return;
  const focal=clamp(Number(camera.focalLengthMm)||35,8,300);
  const sensor=clamp(Number(camera.sensorWidthMm)||36,5,70);
  const angle=fov(sensor,focal);
  ...
  function save(){
    const fl=clamp(Number(E('setSketchFocalLength').value)||35,8,300);
    const sw=clamp(Number(E('setSketchSensorWidth').value)||36,5,70);
    const a=fov(sw,fl),bundle=activeBundle();
    ...
    E('setSketchFovAngle').textContent=a.toFixed(1)+'°';
    E('setSketchFovW3').textContent=widthAt(a,3).toFixed(2)+' m';
    E('setSketchFovW5').textContent=widthAt(a,5).toFixed(2)+' m';
    draw();
  }
  E('setSketchFocalLength').addEventListener('input',save);
  E('setSketchSensorWidth').addEventListener('input',save);
}
function selectFromPointer(ev){
  const g=ev.target&&ev.target.closest?ev.target.closest('[data-object-id]'):null;
  if(!g)return;
  selectedObjectId=g.dataset.objectId||null;
  setTimeout(()=>{draw();enhanceSelected()},0);
}
function selectAfterAction(id){
  setTimeout(()=>{
    const bundle=activeBundle();if(!bundle)return;
    if(id==='setSketchAddCamera'){
      const cams=bundle.scene.objects.filter(o=>o.type==='camera');selectedObjectId=cams.length?cams[cams.length-1].id:null;
    }else if(id==='setSketchApplyMeasurements'){
      const cam=bundle.scene.objects.find(o=>o.type==='camera');selectedObjectId=cam?cam.id:null;
    }else if(id==='setSketchNew'||id==='setSketchCopy'||id==='setSketchDeleteScene'||id==='setSketchSceneSelect'){
      selectedObjectId=null;
    }
    draw();enhanceSelected();
  },0);
}
function install(){
  ...
  if(!E('setSketchCameraFovStyle')){
    ...
    st.textContent='.set-camera-fov-controls{margin-top:10px;padding:10px;background:#10141a;border:1px solid #30343b;border-radius:12px}.set-camera-fov-controls h4{margin:0 0 8px}.set-camera-fov-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.set-camera-fov-grid label{font-size:11px;color:#9299a3}@media(max-width:520px){.set-camera-fov-grid{grid-template-columns:1fr 1fr}}';
    document.head.appendChild(st);
  }
  svg.addEventListener('pointerdown',selectFromPointer);
  svg.addEventListener('pointermove',()=>draw());
  svg.addEventListener('pointerup',()=>{draw();setTimeout(enhanceSelected,0)});
  svg.addEventListener('pointercancel',()=>draw());
  document.addEventListener('click',ev=>{
    const id=String(ev.target&&ev.target.id||'');
    if(id.indexOf('setSketch')!==0)return;
    selectAfterAction(id);
    setTimeout(()=>{draw();enhanceSelected()},0);
  });
  setInterval(()=>{if(E('setSketchSvg'))draw()},700);
  draw();return true;
}
