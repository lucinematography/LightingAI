...
const TXT={
 sr:{
  ...
  tracking:'CAMERA TRACKING',trackSubject:'PRATI GLUMCA',off:'ISKLJUČENO',framing:'TRACKING',framingMode:'FRAMING',framingCenter:'CENTRIRAJ GLUMCA',framingPreserve:'ZADRŽI TRENUTNI FRAMING',framingOffset:'HORIZONTALNI OFFSET',
 ...
 en:{
  ...
  tracking:'CAMERA TRACKING',trackSubject:'TRACK ACTOR',off:'OFF',framing:'TRACKING',framingMode:'FRAMING',framingCenter:'CENTER ACTOR',framingPreserve:'PRESERVE CURRENT FRAMING',framingOffset:'HORIZONTAL OFFSET',
...
function renderControls(){
 ...
 if(o.type==='camera'){
  ...
  const framingMode=b.trackFramingMode==='preserve'?'preserve':'center',offset=Number(b.trackOffsetDeg)||0;
   tracking='<div class="blocking-track blocking-track-quick"><div class="blocking-track-top"><h4>'+esc(x.tracking)+'</h4><label class="blocking-check blocking-check-inline"><input id="blockingTrackingEnabled" type="checkbox" '+(b.trackingEnabled?'checked':'')+'> '+esc(x.framing)+'</label></div><div class="blocking-track-compact"><div class="blocking-track-group"><small>'+esc(x.trackSubject)+'</small><div class="blocking-subject-buttons">'+subjectButtons+'</div></div><div class="blocking-track-group"><small>'+esc(x.framingMode)+'</small><div class="blocking-framing-buttons"><button type="button" class="btn '+(framingMode==='center'?'primary':'secondary')+' blocking-framing-choice" data-framing-mode="center">'+esc(x.framingCenter)+'</button><button type="button" class="btn '+(framingMode==='preserve'?'primary':'secondary')+' blocking-framing-choice" data-framing-mode="preserve">'+esc(x.framingPreserve)+'</button></div></div></div><div class="blocking-framing-readout blocking-framing-readout-inline"><small>'+esc(x.framingOffset)+'</small><b>'+(framingMode==='preserve'?(offset>=0?'+':'')+offset.toFixed(1)+'°':'0.0°')+'</b></div></div><details class="blocking-track blocking-camera-tools"><summary>'+esc(x.cameraTools)+'</summary><div class="blocking-camera-actions"><button id="blockingSaveShot" class="btn primary" type="button">'+esc(x.saveShot)+'</button><button class="btn secondary blocking-save-setup" data-slot="A" type="button">'+esc(x.setupA)+'</button><button class="btn secondary blocking-save-setup" data-slot="B" type="button">'+esc(x.setupB)+'</button><button class="btn secondary blocking-save-setup" data-slot="C" type="button">'+esc(x.setupC)+'</button></div></details>';
 }
 box.innerHTML=tracking+'<div class="blocking-head"><div><small>'+esc(x.select)+'</small><b>'+esc(o.label||o.type)+'</b></div><div id="blockingTime">0.0 / '+maxDuration().toFixed(1)+' s</div></div>'+
 ...
 '<div class="blocking-note">'+esc(x.hint)+'</div>';
...
function install(){
 ...
 if(!E('blockingStyle')){const st=document.createElement('style');st.id='blockingStyle';st.textContent='.blocking-shell{margin-top:14px;padding:12px;background:#0e1217;border:1px solid #3a424d;border-radius:14px}.blocking-shell h3{margin:0;color:#f5c542}.blocking-intro,.blocking-note,.blocking-empty{color:#aab1bb;font-size:14px;line-height:1.5}.blocking-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin:10px 0}.blocking-head small,.blocking-grid small{display:block;color:#aab1bb;font-size:12px}.blocking-head b{display:block;margin-top:3px}.blocking-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.blocking-grid>div,.blocking-grid label{background:#15191e;border:1px solid #30343b;border-radius:10px;padding:11px;font-size:13px}.blocking-grid input{margin-top:5px}.blocking-actions,.blocking-transport{display:grid;grid-template-columns:repeat(2,1fr);gap:7px;margin-top:8px}.blocking-transport{grid-template-columns:repeat(4,1fr)}.blocking-track{margin-top:10px;padding:10px;border:1px solid #30343b;border-radius:11px;background:#12161b}.blocking-track h4{margin:0 0 8px}.blocking-track-quick{margin-top:8px}.blocking-track-top{display:flex;align-items:center;justify-content:space-between;gap:8px}.blocking-track-top h4{margin:0;min-width:0;flex:1}.blocking-check{display:block;margin-top:8px;font-size:14px}.blocking-check-inline{margin-top:0;font-size:12px;white-space:nowrap;display:flex;align-items:center;gap:4px;flex:0 0 auto}.blocking-track-compact{display:grid;grid-template-columns:.8fr 1.2fr;gap:8px;margin-top:8px}.blocking-track-group>small{display:block;color:#9299a3;font-size:10px;margin-bottom:4px}.blocking-subject-buttons{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px}.blocking-framing-buttons{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin-top:0}.blocking-framing-buttons .btn,.blocking-subject-buttons .btn{min-width:0;white-space:normal}.blocking-framing-readout{margin-top:7px;padding:8px;background:#15191e;border:1px solid #30343b;border-radius:9px}.blocking-framing-readout-inline{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 8px}.blocking-framing-readout small{display:block;color:#9299a3;font-size:10px}.blocking-framing-readout b{display:block;color:#89c7ff;margin-top:0}.blocking-camera-tools summary{cursor:pointer;font-weight:800}.blocking-camera-tools[open] summary{margin-bottom:8px}.blocking-camera-actions{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.blocking-shell .btn{font-size:14px;min-height:46px}.blocking-shell select,.blocking-shell input{font-size:16px}.blocking-note{margin-top:8px}@media(max-width:520px){.blocking-intro{display:none}.blocking-shell{padding:9px}.blocking-track-quick{margin-top:6px;padding:8px}.blocking-track-compact{grid-template-columns:1fr;gap:6px}.blocking-track-top{align-items:flex-start}.blocking-track-top h4{font-size:15px}.blocking-check-inline{font-size:11px}.blocking-track-quick .btn{min-height:40px;font-size:12px;padding:7px}.blocking-framing-readout-inline{margin-top:6px}.blocking-camera-tools{padding:8px}.blocking-transport{grid-template-columns:repeat(2,1fr)}}';document.head.appendChild(st)}
...
window.LightingAIBlocking={version:'0.5-compact-tracking',play:play,pause:pause,stop:()=>stop(false),reset:reset,render:renderAll,prepareTrackingOffsets:prepareTrackingOffsets};
