...
function accessoryTreeRecords(fixtureId) {
  const fixture=FIXTURE_LIBRARY.find(f=>f.id===fixtureId); if(!fixture) return [];
...
function accessoryTreeForFixture(fixtureId) {
  return accessoryTreeRecords(fixtureId).map(r=>{
    const parentNames=r.parentIds.map(id=>id===fixtureId?(FIXTURE_LIBRARY.find(f=>f.id===id)?.model||id):(ACCESSORY_LIBRARY.find(a=>a.id===id)?.model||id));
    const dependency=r.depth>1?`dependency level: ${r.depth-1}, requires: ${parentNames.join(' or ')}`:'direct fixture accessory';
    const a=ACCESSORY_LIBRARY.find(x=>x.id===r.id);
...
app.get("/api/fixtures/:id/accessories",(req,res)=>{const fixture=FIXTURE_LIBRARY.find(f=>f.id===req.params.id);if(!fixture)return res.status(404).json({error:"Fixture not found."});res.json({fixture,accessories:accessoryTreeRecords(fixture.id)});});
