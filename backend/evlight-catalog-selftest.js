...
const sp150bi=fixtures.find(x=>x.id==='evlight-sp150bi');
if(!sp150bi) failures.push('Missing EV Light SP150BI control-route fixture');
else{
  const control=sp150bi.control||{};
  for(const item of ['DMX512','RDM']) if(!control.wired?.includes(item)) failures.push('EV Light SP150BI wired control path missing: '+item);
  if(control.wireless?.length) failures.push('EV Light SP150BI must not claim manufacturer-documented wireless control');
  if(control.builtInCRMX) failures.push('EV Light SP150BI must not claim built-in CRMX/LumenRadio');
  if(control.builtInBluetooth) failures.push('EV Light SP150BI must not claim built-in Bluetooth');
  if(control.directLightingAI?.length) failures.push('EV Light SP150BI must not claim direct LightingAI Art-Net/sACN transport');
  if(!control.externalInterfaceRequired?.includes('Wired DMX interface for DMX512/RDM control')) failures.push('EV Light SP150BI wired DMX/RDM interface requirement missing');
  if(!control.unavailableDirectProtocols?.some(x=>String(x).includes('Art-Net or sACN'))) failures.push('EV Light SP150BI network-protocol limitation missing');
  if(!control.unavailableDirectProtocols?.some(x=>String(x).includes('CRMX/LumenRadio'))) failures.push('EV Light SP150BI wireless-protocol limitation missing');
  if(!Array.isArray(control.sourceUrls)||!control.sourceUrls.includes('https://www.evlightprofessional.com/quality-fresnel-led-68836124.html')) failures.push('EV Light SP150BI official model source missing');
...
const sp350bi=fixtures.find(x=>x.id==='evlight-sp350bi');
if(!sp350bi) failures.push('Missing EV Light SP350BI control-route fixture');
else{
  const control=sp350bi.control||{};
  for(const item of ['DMX512','RDM']) if(!control.wired?.includes(item)) failures.push('EV Light SP350BI wired control path missing: '+item);
  if(control.wireless?.length) failures.push('EV Light SP350BI must not claim manufacturer-documented wireless control');
  if(control.builtInCRMX) failures.push('EV Light SP350BI must not claim built-in CRMX/LumenRadio');
  if(control.builtInBluetooth) failures.push('EV Light SP350BI must not claim built-in Bluetooth');
  if(control.directLightingAI?.length) failures.push('EV Light SP350BI must not claim direct LightingAI Art-Net/sACN transport');
  if(!control.externalInterfaceRequired?.includes('Wired DMX interface for DMX512/RDM control')) failures.push('EV Light SP350BI wired DMX/RDM interface requirement missing');
  if(!control.unavailableDirectProtocols?.some(x=>String(x).includes('Art-Net or sACN'))) failures.push('EV Light SP350BI network-protocol limitation missing');
  if(!control.unavailableDirectProtocols?.some(x=>String(x).includes('CRMX/LumenRadio'))) failures.push('EV Light SP350BI wireless-protocol limitation missing');
  if(!Array.isArray(control.sourceUrls)||!control.sourceUrls.includes('https://www.evlightpro.com/fresnel-led/63234789.html')) failures.push('EV Light SP350BI official model source missing');
  if(!Array.isArray(sp350bi.dmxChannels)||!sp350bi.dmxChannels.includes(5)||!sp350bi.dmxChannels.includes(9)) failures.push('EV Light SP350BI DMX channel options must include 5 and 9');
  if(Number(sp350bi.cctK?.min)!==2700||Number(sp350bi.cctK?.max)!==6400) failures.push('EV Light SP350BI CCT range must be 2700-6400K');
  if(sp350bi.zoom!=='Electric zoom / manual zoom optional') failures.push('EV Light SP350BI zoom detail missing');
}
const sp350fc=fixtures.find(x=>x.id==='evlight-sp350fc');
if(!sp350fc) failures.push('Missing EV Light SP350FC control-route fixture');
else{
  const control=sp350fc.control||{};
  for(const item of ['DMX512','RDM']) if(!control.wired?.includes(item)) failures.push('EV Light SP350FC wired control path missing: '+item);
  if(control.wireless?.length) failures.push('EV Light SP350FC must not claim manufacturer-documented wireless control');
  if(control.builtInCRMX) failures.push('EV Light SP350FC must not claim built-in CRMX/LumenRadio');
  if(control.builtInBluetooth) failures.push('EV Light SP350FC must not claim built-in Bluetooth');
  if(control.directLightingAI?.length) failures.push('EV Light SP350FC must not claim direct LightingAI Art-Net/sACN transport');
...
const sp600=fixtures.find(x=>x.id==='evlight-sp600');
if(!sp600) failures.push('Missing EV Light SP600 control-route fixture');
else{
  const control=sp600.control||{};
  for(const item of ['DMX512','RDM']) if(!control.wired?.includes(item)) failures.push('EV Light SP600 wired control path missing: '+item);
  if(control.wireless?.length) failures.push('EV Light SP600 must not claim manufacturer-documented wireless control');
  if(control.builtInCRMX) failures.push('EV Light SP600 must not claim built-in CRMX/LumenRadio');
  if(control.builtInBluetooth) failures.push('EV Light SP600 must not claim built-in Bluetooth');
  if(control.directLightingAI?.length) failures.push('EV Light SP600 must not claim direct LightingAI Art-Net/sACN transport');
  if(!control.externalInterfaceRequired?.includes('Wired DMX interface for DMX512/RDM control')) failures.push('EV Light SP600 wired DMX/RDM interface requirement missing');
  if(!control.unavailableDirectProtocols?.some(x=>String(x).includes('Art-Net or sACN'))) failures.push('EV Light SP600 network-protocol limitation missing');
  if(!control.unavailableDirectProtocols?.some(x=>String(x).includes('CRMX/LumenRadio'))) failures.push('EV Light SP600 wireless-protocol limitation missing');
  if(!Array.isArray(control.sourceUrls)||!control.sourceUrls.includes('https://www.evlightprofessional.com/products-index/2/')) failures.push('EV Light SP600 official model source missing');
  if(!Array.isArray(sp600.dmxChannels)||!sp600.dmxChannels.includes(2)) failures.push('EV Light SP600 DMX channel option must include 2');
  if(sp600.dmxConnection!=='3-pin XLR in/out') failures.push('EV Light SP600 DMX connector detail missing');
  if(sp600.zoom!=='Manual zoom') failures.push('EV Light SP600 zoom detail missing');
}
