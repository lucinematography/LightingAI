...
function currentSetup(){
 ...
 const focal=cam&&Number(cam.focalLengthMm)||35,sensor=cam&&Number(cam.sensorWidthMm)||36,hfov=2*Math.atan(sensor/(2*focal))*180/Math.PI;let framing=null;try{if(window.LightingAICameraFov&&typeof window.LightingAICameraFov.snapshot==='function')framing=window.LightingAICameraFov.snapshot()}catch(e){}
 return {sceneId:sc&&sc.id||null,sceneName:sc&&sc.name||null,cameraId:cam&&cam.id||null,cameraLabel:cam&&(cam.label||'Kamera')||null,cameraX:cam?Number(cam.x):null,cameraY:cam?Number(cam.y):null,cameraRot:cam?(Number(cam.rot)||0):null,focalLengthMm:cam?focal:null,sensorWidthMm:cam?sensor:null,horizontalFovDeg:cam?hfov:null,subjectId:subj&&subj.id||null,subjectLabel:subj&&(subj.label||'Glumac')||null,cameraToSubjectM:distance>0?distance:null,framing:framing?JSON.parse(JSON.stringify(framing)):null,blocking:cam&&cam.blocking?JSON.parse(JSON.stringify(cam.blocking)):null,dof:dof||null,flicker:flicker||null,dmx:dmx||null,continuity:continuity||null,createdAt:new Date().toISOString()};
}
...
function render(){
 ...
 box.innerHTML=state.rows.map((r,i)=>{const s=r.setup||{},ref=r.referenceImage;
  ...
  '<textarea class="shot-notes" data-id="'+r.id+'" placeholder="'+x.notes+'">'+esc(r.notes||'')+'</textarea><div class="actions"><button class="btn primary shot-restore" data-id="'+r.id+'" type="button">'+x.restore+'</button><button class="btn secondary shot-refresh" data-id="'+r.id+'" type="button">'+x.refresh+'</button><button class="btn secondary shot-dup" data-id="'+r.id+'" type="button">'+x.duplicate+'</button><button class="btn secondary shot-remove" data-id="'+r.id+'" type="button">'+x.remove+'</button></div></div>';
 }).join('');summary();
}
...
function restoreSetup(rowId){const r=state.rows.find(x=>x.id===rowId),s=r&&r.setup;if(!s)return false;const raw=parse(SET_KEY,null);if(!raw||!Array.isArray(raw.scenes))return false;const sc=raw.scenes.find(x=>x.id===s.sceneId);if(!sc||!Array.isArray(sc.objects))return false;let cam=sc.objects.find(o=>o.id===s.cameraId&&o.type==='camera');if(!cam&&s.cameraLabel)cam=sc.objects.find(o=>o.type==='camera'&&(o.label||'Kamera')===s.cameraLabel);if(!cam)return false;if(Number.isFinite(Number(s.cameraX)))cam.x=Number(s.cameraX);if(Number.isFinite(Number(s.cameraY)))cam.y=Number(s.cameraY);if(Number.isFinite(Number(s.cameraRot)))cam.rot=Number(s.cameraRot);if(Number.isFinite(Number(s.focalLengthMm)))cam.focalLengthMm=Number(s.focalLengthMm);if(Number.isFinite(Number(s.sensorWidthMm)))cam.sensorWidthMm=Number(s.sensorWidthMm);if(s.blocking&&typeof s.blocking==='object')cam.blocking=JSON.parse(JSON.stringify(s.blocking));raw.activeId=sc.id;localStorage.setItem(SET_KEY,JSON.stringify(raw));try{const a=window.LightingAISetSketch;if(a&&typeof a.selectObject==='function')a.selectObject(cam.id);if(a&&typeof a.refresh==='function')a.refresh()}catch(e){}try{if(window.LightingAICameraSetups&&typeof window.LightingAICameraSetups.selectCamera==='function')window.LightingAICameraSetups.selectCamera(cam.id)}catch(e){}return true}
...
function bindRows(){
 ...
 root.addEventListener('click',ev=>{
  ...
  const restore=ev.target.closest('.shot-restore');if(restore){const ok=restoreSetup(restore.dataset.id);const card=restore.closest('.shot-list-row');if(card){let note=card.querySelector('.shot-restore-status');if(!note){note=document.createElement('div');note.className='shot-restore-status';note.style.cssText='font-size:11px;color:#b8f0d1;margin-top:6px';card.appendChild(note)}note.textContent=ok?t().restored:t().restoreFail;setTimeout(()=>{if(note)note.textContent=''},2200)}return}
...
window.LightingAIShotList={version:'1.2-shot-restore',snapshot:function(){return JSON.parse(JSON.stringify(state))},addCurrent:add,refreshCurrentSetup:refreshSetup,restoreSetup:restoreSetup,render:render};
