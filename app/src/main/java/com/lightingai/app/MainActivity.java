...
public class MainActivity extends Activity {
    ...
    private void installCatalogView() {
        ...
        String js = "(function(){" +
            ...
            "var h='<div class=\\\"card\\\"><b>'+(sr?'PRETRAGA KATALOGA':'CATALOG SEARCH')+'</b><input id=\\\"catalogSearch\\\" style=\\\"margin-top:10px\\\" placeholder=\\\"'+(sr?'Pretraži svetla, kablove, klape, softboxe, dodatke...':'Search fixtures, cables, barn doors, softboxes, accessories...')+'\\\" value=\\\"'+esc(q)+'\\\" oninput=\\\"renderFullCatalog()\\\"></div>';" +
            ...
            "h+='<details class=\\\"card\\\"><summary style=\\\"font-weight:800;font-size:18px;cursor:pointer\\\">'+(sr?'DODATNA OPREMA':'ACCESSORIES')+' ('+as.length+')</summary><div style=\\\"margin-top:12px\\\">';as.forEach(function(a){var on=isSelected(a.id);var compat=(a.compatibleWith||[]).join(', ');h+='<div style=\\\"margin-bottom:8px\\\"><button class=\\\"btn '+(on?'primary':'secondary')+'\\\" style=\\\"width:100%;text-align:left\\\" onclick=\\\"toggleCatalogItem(\\\'accessory\\\',\\\''+esc(a.id)+'\\\')\\\">'+(on?'✓ ':'＋ ')+esc(nameAccessory(a))+'</button>'+(compat?'<div class=\\\"muted small\\\" style=\\\"padding:4px 8px\\\">'+(sr?'Kompatibilno sa: ':'Compatible with: ')+esc(compat)+'</div>':'')+'</div>';});h+='</div></details>';box.innerHTML=h;};" +
