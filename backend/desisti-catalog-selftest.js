...
{
  const fixture=fixtures.find(item=>item.id==='desisti-super-led-f14hp-vw');
  const mode16=fixture?.dmxModes?.find(mode=>mode?.name==='Vari-White 16-bit');
  if(!mode16||mode16.channels!==4||mode16.verified!==true||mode16.sourceUrl!==f14HpVwWidthSource){
    failures.push('Incorrect verified F14 HP VW 16-bit DMX width/source');
  }
  for(const key of ['controls','requiredChannels']){
    if(mode16?.[key]!=null&&(!Array.isArray(mode16[key])||mode16[key].length)){
      failures.push('F14 HP VW 16-bit channel mapping must remain hidden until sourced: '+key);
    }
  }
}
