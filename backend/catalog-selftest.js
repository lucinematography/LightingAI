...
for (const fixture of RUNTIME_CATALOG.fixtures) {
  if (RUNTIME_CATALOG.fixtureById.get(fixture.id) !== fixture) failures.push(`Fixture map mismatch: ${fixture.id}`);
}
for (const accessory of RUNTIME_CATALOG.accessories) {
  ...
  for (const target of accessory.compatibleWith || []) {
    if (!fixtureIds.has(target) && !accessoryIds.has(target)) failures.push(`Unresolved target: ${accessory.id} -> ${target}`);
  }
}

function reachesFixture(id, trail = new Set()) {
  if (fixtureIds.has(id)) return true;
  if (trail.has(id)) return false;
  const accessory = RUNTIME_CATALOG.accessoryById.get(id);
  if (!accessory) return false;
  const next = new Set(trail); next.add(id);
  return (accessory.compatibleWith || []).some(target => reachesFixture(target, next));
}
for (const accessory of RUNTIME_CATALOG.accessories) {
  if (!reachesFixture(accessory.id)) failures.push(`No fixture path: ${accessory.id}`);
}

if (RUNTIME_CATALOG.duplicateAccessoryIds.length) failures.push(`Duplicate runtime accessory ids: ${RUNTIME_CATALOG.duplicateAccessoryIds.join(', ')}`);

const uniqueFailures = [...new Set(failures)];
console.log(JSON.stringify({
  ok: uniqueFailures.length === 0,
  fixtures: RUNTIME_CATALOG.fixtures.length,
  accessories: RUNTIME_CATALOG.accessories.length,
