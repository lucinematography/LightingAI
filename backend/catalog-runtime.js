...
export function buildRuntimeCatalog() {
  ...
  return { fixtures, accessories, kits, sourceAccessoryDefinitions: accessoryDefinitions, duplicateAccessoryIds: [...new Set(duplicateAccessoryIds)], fixtureById: new Map(fixtures.map(f => [f.id, f])), accessoryById: new Map(accessories.map(a => [a.id, a])), kitById: new Map(kits.map(k => [k.id, k])) };
}
