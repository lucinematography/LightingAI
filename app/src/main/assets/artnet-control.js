...
const TXT={
 sr:{
  ...
  protocol:'MREŽNI PROTOKOL',artnet:'Art-Net',sacn:'sACN (E1.31)',sacnInfo:'sACN automatski koristi multicast adresu za izabrani Universe.',target:'ART-NET NODE / IP',artnetAutoHint:'AUTO koristi directed broadcast aktivne IPv4 mreže; pronađeni node / IP koristi unicast.',discover:'PRONAĐI ART-NET NODE-OVE',nodes:'PRONAĐENI NODE-OVI',noNodes:'Nema pronađenih Art-Net node-ova.',discovering:'Tražim Art-Net node-ove…',patchDevice:'UREĐAJ IZ DMX PATCH-A',manual:'Ručno / bez Patch uređaja',refresh:'OSVEŽI PATCH',
...
function finishArmPreflight(id,payload,error){
 ...
 if(error||!native)failure=t().preflightFailed;
  ...
  if(auto&&!interfaces.some(item=>item&&item.broadcast))failure=t().preflightNoBroadcast;
...
function translate(){
 ...
 const x=t();
 E('artnetTitle').textContent=x.title;E('artnetIntro').textContent=x.intro;E('networkDmxProtocolLabel').textContent=x.protocol;E('networkDmxProtocol').options[0].text=x.artnet;E('networkDmxProtocol').options[1].text=x.sacn;E('artnetTargetLabel').textContent=x.target;E('artnetAutoHint').textContent=x.artnetAutoHint;E('artnetDiscover').textContent=x.discover;
 ...
 updateProtocolUi();renderPatchDevices();renderMasterControl();renderMasterCctControl();renderMasterRgbControl();renderControlGroups();renderScenes();renderCueStack();renderDiagnostics(null,'');
