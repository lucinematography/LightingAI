...
function render(){
  ...
  box.innerHTML=row(x.mode,mode,modeGood)+row(x.camera,cameraText,c.rearCameraCount>0||(!c.nativeAndroid&&c.cameraAny))+row(x.tilt,tiltLabel(c.tiltMode),c.tiltMode!=='none')+row(x.location,locParts.join(' · '),c.gps||c.networkLocation)+row(x.depth,c.depthOutput?x.depthYes:x.depthNo,null)+row(x.system,systemLabel(c),null);
...
function init(){
  ...
  style.textContent='.device-cap-grid{display:grid;gap:0;margin-top:8px}.device-cap-row{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #292d33;font-size:12px}.device-cap-row span{color:#9299a3}.device-cap-row b{text-align:right;color:#d9dde3}.device-cap-row b.good{color:#b8f0d1}.device-cap-row b.limited{color:#f5dd91}.device-cap-note,.device-cap-report-info,.device-cap-report-status{font-size:11px;line-height:1.45;color:#9299a3;margin:10px 0 0}.device-cap-report-status{min-height:16px;color:#b8f0d1}';document.head.appendChild(style);
